...
import com.community.entity.po.Repair;
import com.community.entity.vo.RepairVo;
import com.community.service.IRepairService;
...
//        Assert.notNull(userRepository.selectById(repair.getUserId()), "该用户不存在");
//        QueryWrapper<UserRole> queryWrapper = new QueryWrapper<>();
//        queryWrapper.eq(UserRole.USER_ID, repair.getUserId());
//        Assert.state(userRoleRepository.selectOne(queryWrapper).getRoleId() == 2, "该用户不是住户");
//
//        Assert.state(repairRepository.insert(repair) == 1, "新增失败");
        return true;
    }
...
//        Assert.state(repairRepository.updateById(repair) == 1, "更新失败");
...
//        repairPage = repairRepository.selectPage(repairPage, queryWrapper);
//
...
//                HouseholdInfo householdInfo = householdInfoRepository.selectOne(householdInfoQueryWrapper);
...
//                    StaffInfo staffInfo = staffInfoRepository.selectOne(staffInfoQueryWrapper);
//
